            ...
            <div class="space-y-4">
                ...
                <div>
                    ...
                    <input
                        ...
                        class="w-full rounded-lg border border-gray-300 bg-white px-3 py-2 text-sm outline-none transition-all focus:border-gray-400 focus:ring-1 focus:ring-gray-400"
                        placeholder="Digite o email do assinante"
                    >
                </div>
            </div>
            ...
            <x-slot:footer>
                <div class="w-full flex justify-between items-center">
                    ...
                    <div class="flex items-center space-x-2">
                        <button
                            wire:click="$set('subscriberModal', false)"
                        ...
                        <button
                            wire:click="submitFormSubscriber"
                            wire:loading.attr="disabled"
                            wire:target="submitFormSubscriber"
                        ...
                        >
                            <i class="fad fa-save mr-2" wire:loading.remove wire:target="submitFormSubscriber"></i>
                            <i class="fad fa-spinner-third fa-spin mr-2" wire:loading wire:target="submitFormSubscriber"></i>
                            <span wire:loading.remove wire:target="submitFormSubscriber">Salvar</span>
                            <span wire:loading wire:target="submitFormSubscriber">Processando...</span>
                        </button>
                    </div>
                </div>
            </x-slot:footer>
            ...
            <tbody class="bg-white divide-y divide-gray-200">
                ...
                <tr class="hover:bg-gray-50">
                    <td class="px-6 py-4 whitespace-nowrap">
                        <div class="flex items-center">
                            <div class="w-8 h-8 rounded-lg bg-white shadow-sm border border-gray-200 flex items-center justify-center mr-3">
                                <i class="fad fa-user-circle text-gray-800"></i>
                            </div>
                            <div class="text-sm font-medium text-gray-900">
                                @if(empty($subscriber->sub_name))
                    ...
                    <td class="px-6 py-4 text-left whitespace-nowrap">
                        <div class="text-sm text-gray-500">{{ $subscriber->sub_email }}</div>
                    </td>
                    <td class="px-6 py-4 whitespace-nowrap text-right">
                        <div class="w-full flex flex-row items-end justify-end">
                            <button
                                wire:click="edit({{ $subscriber->sub_id }})"
                                ...
                                wire:loading.attr="disabled"
                                wire:target="edit({{ $subscriber->sub_id }})"
                            >
                                <i class="fad fa-edit" wire:loading.remove wire:target="edit({{ $subscriber->sub_id }})"></i>
                                <i class="fad fa-spinner-third fa-spin" wire:loading wire:target="edit({{ $subscriber->sub_id }})"></i>
                            </button>
                            <button
                                wire:click="delete({{ $subscriber->sub_id }})"
                                ...
                                wire:loading.attr="disabled"
                                wire:target="delete({{ $subscriber->sub_id }})"
                            >
                                <i class="fad fa-trash-alt" wire:loading.remove wire:target="delete({{ $subscriber->sub_id }})"></i>
                                <i class="fad fa-spinner-third fa-spin" wire:loading wire:target="delete({{ $subscriber->sub_id }})"></i>
                            </button>
                        </div>
                    </td>
                </tr>
            ...
            <div class="hidden sm:flex-1 sm:flex sm:items-center sm:justify-between">
                <div>
                    <p class="text-sm text-gray-700">
                        ...
                        até
                        <span class="font-medium">{{ count($subscribers) }}</span>
                        de
                        <span class="font-medium">{{ count($subscribers) }}</span>
                        resultados
                    </p>
