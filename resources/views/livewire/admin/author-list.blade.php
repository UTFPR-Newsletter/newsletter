            ...
            <div class="space-y-4">
                ...
                <div>
                    ...
                    <input
                        type="text"
                        id="cpf"
                        wire:model="authorForm.cpf"
                        class="w-full rounded-lg border border-gray-300 bg-white px-3 py-2 text-sm outline-none transition-all focus:border-gray-400 focus:ring-1 focus:ring-gray-400"
                        placeholder="Digite o CPF do autor"
                    >
                </div>
                ...
                <!-- Biografia -->
                <div>
                    <label for="body" class="block text-sm font-medium text-gray-700 mb-1">Biografia</label>
                    <textarea
                        id="body"
            ...
            <x-slot:footer>
                <div class="w-full flex justify-between items-center">
                    ...
                    <div class="flex items-center space-x-2">
                        <button
                            wire:click="$set('authorModal', false)"
                        ...
                        <button
                            wire:click="submitFormAuthor"
                            wire:loading.attr="disabled"
                            wire:target="submitFormAuthor"
                        ...
                        >
                            <i class="fad fa-save mr-2" wire:loading.remove wire:target="submitFormAuthor"></i>
                            <i class="fad fa-spinner-third fa-spin mr-2" wire:loading wire:target="submitFormAuthor"></i>
                            <span wire:loading.remove wire:target="submitFormAuthor">Salvar</span>
                            <span wire:loading wire:target="submitFormAuthor">Processando...</span>
                        </button>
                    </div>
                </div>
            </x-slot:footer>
                    ...
                    <td class="px-6 py-4 whitespace-nowrap">
                        <div class="flex items-center">
                            <div class="w-8 h-8 rounded-lg bg-white shadow-sm border border-gray-200 flex items-center justify-center mr-3">
                                <i class="fad fa-user text-gray-800"></i>
                            </div>
                            <div class="text-sm font-medium text-gray-900">{{ $author->aut_name }}</div>
                        </div>
                    </td>
                    <td class="px-6 py-4 text-left whitespace-nowrap">
                        <div class="text-sm text-gray-500">{{ $author->aut_cpf }}</div>
                    </td>
                    <td class="px-6 py-4 text-left">
                        <div class="text-sm text-gray-500">
                            @if(empty($author->aut_body))
                                <span class="italic text-gray-400">
                                    <i class="fad fa-info-circle mr-1 text-xs"></i>
                                    Sem biografia definida
                                </span>
                            @else
                                {{ Str::limit($author->aut_body, 100) }}
                            @endif
                        </div>
                    </td>
                    <td class="px-6 py-4 whitespace-nowrap text-right">
                        <div class="w-full flex flex-row items-end justify-end">
                            <button
                                ...
                                wire:loading.attr="disabled"
                                wire:target="edit({{ $author->aut_id }})"
                            >
                                <i class="fad fa-edit" wire:loading.remove wire:target="edit({{ $author->aut_id }})"></i>
                                <i class="fad fa-spinner-third fa-spin" wire:loading wire:target="edit({{ $author->aut_id }})"></i>
                            </button>
                            <button
                                wire:click="delete({{ $author->aut_id }})"
                                ...
                                wire:loading.attr="disabled"
                                wire:target="delete({{ $author->aut_id }})"
                            >
                                <i class="fad fa-trash-alt" wire:loading.remove wire:target="delete({{ $author->aut_id }})"></i>
                                <i class="fad fa-spinner-third fa-spin" wire:loading wire:target="delete({{ $author->aut_id }})"></i>
                            </button>
                        </div>
                    </td>
                </tr>
            ...
            <div class="hidden sm:flex-1 sm:flex sm:items-center sm:justify-between">
                <div>
                    <p class="text-sm text-gray-700">
                        ...
                        até
                        <span class="font-medium">{{ count($authors) }}</span>
                        de
                        <span class="font-medium">{{ count($authors) }}</span>
                        resultados
                    </p>
